...
export default class Setting extends React.Component {
  ...
  render() {
    const { item, logOut, rateApp } = this.props;
    const {
      colorFontSetting,
      chevron,
      pressing,
      ...
      <Translation>
        ...
        <View>
          ...
          {pressing && item.text === 'Notifications' ? (
            <List.Item title="Programar alertas" onPress={()=>console.log("working!")}/>
          ) : null}
          {pressing && item.text === 'More' ? (
            <>
            <List.Item title={t("Designed and developed")} titleStyle={{fontWeight: 'bold'}} />
            <List.Item title="Bryan González Arellano" />
            <List.Item title="Alejandro Castro Reus" />
            </>
          ) : null}
          {pressing && item.text === 'Rate us' &&               <List.Item 
                title={t("Play Store")}
                onPress={() => rateApp()} />}
        </View>
        )}
