...
class ProgressHome extends React.Component {
  ...
  constructor(props) {
    ...
    fillProgress(item) {
      const {
        cuantification, quantityCuantification, daysToDo, repetition, initialDate
      } = item;   
      const today = new Date();
      const [year, mth, date] = getCurrentDate(repetition, today, new Date(initialDate));
      if (!daysToDo[year] || !daysToDo[year][mth] || !daysToDo[year][mth][date] || daysToDo[year][mth][date].status === 'none') { 
        return false;
      } 
      ...
      return ((amount / quantityCuantification) * 100) !== 100;
    }
    ...
    listTemplate = (item) => (
      ...
      {(t, { i18 }) => (
        ...
        >
          ...
          {this.fillProgress(item) ? <MaterialCommunityIcons name="close" size={30} color="red" /> : <MaterialCommunityIcons name="check" size={30} color="green" /> }
        </TouchableOpacity>
      )}
      </Translation>
    )
            ...
            >
              ...
              {list.map((l) => (
                ...
                >
                  ...
                  <Text style={[styles.bottomModalText, l.titleStyle]}>{t(l.title)}</Text>
                </TouchableOpacity>

              ))}
            </BottomSheet>
            <Button onPress={() => removeGoals()}>{t("Delete goals")}</Button>
          </View>
         )}
        </Translation>
      );
