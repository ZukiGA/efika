...
const createTask = (state) => {
  ...
  try {
    ...
    switch (repetition) {
      case WEEK:
        [daysToDo, finalDate] = calculateWeeksMonths(timesToDo, cuantification, WEEK);
        break;
      case MONTH:
        [daysToDo, finalDate] = calculateWeeksMonths(timesToDo, cuantification, MONTH);
        break;
      default:
        [daysToDo, finalDate] = calculateDays(days, timesToDo, cuantification);
        break;
    }

    const initialDate = new Date();
    const currentDate = new Date();
    const newLocal = initialDate / (1000);
