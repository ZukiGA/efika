...
export default class App extends React.Component {
  ...
  chooseStyle = (day) => {
    ...
    if (typeof daysGoal[year] !== 'undefined' && typeof daysGoal[year][month] !== 'undefined' && typeof daysGoal[year][month][day] !== 'undefined') {
      switch (daysGoal[year][month][day][STATUS]) {
    ...
    return (
      <View style={styles.container}>
        ...
        <View style={styles.header}>
          {days.map((elem) => (
            <View key={elem} style={styles.headerBox}>
              <Text>{elem}</Text>
            </View>
          ))}
        </View>
            ...
            <View key={week} style={styles.calendarWeek}>
              {week.map((day) => {
                return (
                  day !== 0
                    ? (
                      <View key={day} style={[styles.box, this.chooseStyle(day)]}>
                        <Text>
                         {t(day)}
                        </Text>
                      </View>
