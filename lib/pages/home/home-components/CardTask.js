...
const CardTask = (props) => {
  ...
  const { mainColor, title, completedTimes, timesToDo } = item;
  ...
  const textProgress = () => {
    ...
    if (!daysToDo[year] || !daysToDo[year][mth] || !daysToDo[year][mth][date])
      return 100;
    ...
    const percentage = Math.floor((amount / quantityCuantification) * 100);
