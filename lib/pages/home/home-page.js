...
class HomePage extends React.Component {
    ...
    this.setState({
      carouselItems: carouselItems.map((elem) => {
        if (elem.id === id) {
          ...
          elem.daysToDo[today.getFullYear()][today.getMonth()][today.getDate()].status = 'completed';
          return elem;
        }
        return elem;
      })
  ...
  changeCuantification = (idTask, quantity) => {
    ...
    this.setState({
      carouselItems: carouselItems.map((elem) => {
        if (elem.id === idTask) {
          elem.daysToDo[today.getFullYear()][today.getMonth()][today.getDate()].amount += quantity;
          return elem;
        }
        return elem;
      })
