...
import {
  addTask, removeTask, continueTask, changeCuantification
} from '../../actions/user';
...
class HomePage extends React.Component {
  ...
  changeCuantification = (idTask, quantity) => {
    ...
    const today = new Date();
    const [year, month, date] = [today.getFullYear(), today.getMonth(), today.getDate()];
    changeReduxCuantification(idTask, quantity, year, month, date);
    this.setState({
      carouselItems: carouselItems.map((elem) => {
        if (elem.id === idTask) {
          if (elem.daysToDo[year][month][date].amount + quantity
        ...
        <View style={styles.bottomScreenStyle}>
          <View style={styles.textGoalsSection}>
            <View style={styles.frameYourGoals}>
              <Text style={styles.textYourGoals}>Tus metas: </Text>
            </View>
