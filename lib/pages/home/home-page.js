    ...
    this.setState({
      carouselItems: carouselItems.map((elem) => {
        if (elem.id === idTask) {
          ...
          if (elem.daysToDo[year][month][date].amount + quantity >= elem.quantityCuantification) {
            elem.daysToDo[year][month][date].status = 'completed';
          }
          return elem;
        }
...
const mapDispatchToProps = (dispatch) => {
  return bindActionCreators({ addNewTask: addTask, removeReduxTask: removeTask }, dispatch);
};
