...
class FormPage extends React.Component {
  ...
  changeModalTimeActive = () => {
    this.setState({ modalTimeActive: !this.state.modalTimeActive });
  };

  openModal = (title, elementToChange) => {
    if (
      (this.state.elemCuantification[0].status &&
        this.state.elemCuantification[0].key == elementToChange) ||
      (this.state.elemCuantification[1].status &&
        this.state.elemCuantification[1].key == elementToChange)
    ) {
      this.setState({
        repeat: 0,
        time: 0,
  ...
  addNewTaskRedux = (taskName, period, activeDays) => {
    ...
    if (period === 'week') {
      ...
      activeDaysMap = activeDays.map((day) => {
        return {
          day,
          color: 'gray',
          percentage: 1,
  ...
  addTask = () => {
    ...
    try {
      ...
      var repetition = this.state.elemRepetition.find(
        (elem) => elem.status === true,
      );
      if (repetition === undefined)
        throw 'No escogiste cuanto vas a repetir tu hábito';
      repetition = repetition.key;
      if (repetition === 'day') {
        var days = this.state.days.flatMap((elem) =>
          elem.status === true ? [elem.key] : [],
        );
    ...
    this.addNewTaskRedux(task.name, repetition, days);

    if (task.repetition === 'day') task.days = days;
    if (
      this.state.elemCuantification[0].status === true ||
      this.state.elemCuantification[1].status === true
    ) {
              ...
              <View style={styles.habitsIconsView}>
                ...
                >
                  <MaterialCommunityIcons
                    ...
                    size={75}
                    color={
                      this.state.elemRepetition[1].status
                        ? '#1389CE'
                        : '#4B4B4B'
                    }
                  />
                ...
                >
                  <MaterialCommunityIcons
                    ...
                    size={75}
                    color={
                      this.state.elemRepetition[2].status
                        ? '#1389CE'
                        : '#4B4B4B'
                    }
                  />
                ...
                <View style={styles.cuantificacionViewStyle}>
                  ...
                  {this.state.elemCuantification[0].status ? (
                    <View>
                      <Text>
                        {' '}
                        Tiempo: {this.state.time[0]} : {this.state.time[1]}
                      </Text>
                    </View>
                  ) : null}
                  {this.state.elemCuantification[1].status ? (
