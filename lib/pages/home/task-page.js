...
const choosePeriodTranslation = (period) => {
  if (period === "day") return "Progress from today";
  if (period === "week") return "Progress from this week";
  return "Progress from this month";
};
...
export default class TaskPage extends React.Component {
  ...
  enableContinue() {
    const { state } = this;
    const { daysToDo, year, mth, date } = state;
    if (
      !daysToDo[year] ||
      !daysToDo[year][mth] ||
      !daysToDo[year][mth][date] ||
      daysToDo[year][mth][date].status === "none" ||
      daysToDo[year][mth][date].status === "completed"
    ) {
      return false;
    }
    return true;
  ...
  retrieveCuantification() {
    const { state } = this;
    const { daysToDo, cuantification, year, mth, date } = state;
    if (
      cuantification &&
      daysToDo[year] &&
      daysToDo[year][mth] &&
  ...
  renderContinueButton() {
    ...
    if (!this.enableContinue()) {
      return <Text> {t("Completed for today")} </Text>;
    }
    if (cuantification === "none") {
      return (
      ...
      <Translation>
        {(t, { i18 }) => (
          <ScrollView style={styles.taskScreenStyles}>
            ...
            />
            <View
              style={[styles.topSquare, { backgroundColor: brighterColor }]}
            />
            ...
            <View style={styles.continue}>
              {this.enableContinue() && cuantification === "time" ? (
                <TouchableOpacity
                  onPress={() => this.setState({ modalActiveTime: true })}
                >
              ...
              {this.enableContinue() && cuantification === "repeat" ? (
                <TouchableOpacity
                  onPress={() => this.setState({ modalActiveRepeat: true })}
                >
                  <MaterialCommunityIcons name="repeat" size={55} />
                </TouchableOpacity>
              ) : null}
              <View>{this.renderContinueButton()}</View>
            </View>
            ...
            {repetition === "day" ? (
              <View style={styles.tabsGrouping}>
                <TouchableOpacity
                  ...
                  onPress={() =>
                    this.setState({ tabWeek: true, tabCalendar: false })
                  }
                >
                  <Text
                    style={
                      tabWeek ? styles.textTabActive : styles.textTabInactive
                    }
                  >
                    {t("Day")}
                  </Text>
                ...
                <TouchableOpacity
                  ...
                  onPress={() =>
                    this.setState({ tabWeek: false, tabCalendar: true })
                  }
                >
                  <Text
                    style={
              ...
              >
                <Text style={styles.textTabActive}>
                  {repetition === "week" ? t("Week") : t("Month")}
                </Text>
              </View>
            )}
            ...
            {tabCalendar ? (
              ...
              <View style={styles.progressView}>
                <Text>{t(choosePeriodTranslation(repetition))}:</Text>
